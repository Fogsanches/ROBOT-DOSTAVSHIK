...
void FooEngine::handleCommand(const std::string& input) {
    ...
    if (cmd == "forward") fsm.onCommand("forward");
    else if (cmd == "left" || cmd == "right") fsm.onCommand("turn");
    else if (cmd == "stop") fsm.onCommand("stop");
    else if (cmd == "go") fsm.onCommand("go");
    else if (cmd == "arrived") fsm.onCommand("arrived");

    std::cout << "FSM state: " << fsm.getStateName() << std::endl;
...
void FooEngine::forward(int time_ms) {
    ...
    std::system("gpio -g write 12 1"); // IN1
    std::system("gpio -g write 13 0"); // IN2
    std::system("gpio -g write 20 0"); // IN3 (p20)
    std::system("gpio -g write 21 1"); // IN4 (p21)
...
void FooEngine::left(int time_ms) {
    ...
    std::system("gpio -g write 12 0"); // IN1
    std::system("gpio -g write 13 0"); // IN2
    std::system("gpio -g write 20 0"); // IN3
    std::system("gpio -g write 21 1"); // IN4
    simulateDelay(time_ms);
    stop();
}
...
void FooEngine::right(int time_ms) {
    ...
    std::system("gpio -g write 13 0"); // IN2
    std::system("gpio -g write 20 0"); // IN3
    std::system("gpio -g write 21 0"); // IN4
    simulateDelay(time_ms);
    stop();
}
